...
def main_page():
    ...
    **What you can do:**
    ...
    st.caption("Built with ❤️ using DuckDB + Streamlit + Electron")

    
if cache_loaded := st.session_state.get("cache_loaded", False) is False:
    cache = dl.load_cache(cache_file="file_cache.json")
...
with st.sidebar:
    ...
    st.image("./assets/reveal-logo-3.png", width=200)

pg = st.navigation([
    st.Page(main_page, title="Duckboard", icon="🦆"),
    st.Page("pages/1_Manage Files.py", icon="📁"),
    st.Page("pages/2_Query Data.py", icon="📊"),
    st.Page("pages/3_Custom Scripts.py", icon="🧩"),
])
pg.run()
